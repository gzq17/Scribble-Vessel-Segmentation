...
def resize(image, target, size):
    ...
    if "masks" in target:
        ...
        mask = F.resize(interpolate_mask, rescaled_size, interpolation = InterpolationMode.NEAREST)#PIL.Image.NEAREST
...
class RandomRotate(object):
    ...
    def __call__(self, img, target):
        ...
        img = img.copy()
        rotated_img = F.rotate(torch.from_numpy(img), angle, InterpolationMode.NEAREST, self.expand, self.center)
        rotated_img = rotated_img.numpy()
        # if "masks" in target:
        mask = target['masks']
        mask = mask.copy()
        rotated_mask = F.rotate(torch.from_numpy(mask), angle, InterpolationMode.NEAREST, self.expand, self.center)
